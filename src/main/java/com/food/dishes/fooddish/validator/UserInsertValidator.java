...
import com.food.dishes.fooddish.command.UserInsertCommand;
...
public class UserInsertValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (passwordConfirm.trim().equals("")) {
            errors.rejectValue("passwordConfirm", "passwordConfirm:required", "비밀번호 확인을 입력해주세요.");
        }
        if (name.trim().equals("")) {
        ...
        if (subAddress.trim().equals("")) {
            errors.rejectValue("subAddress", "subAddress:required", "상세주소를 입력해주세요.");
        }
        if (phone1.trim().equals("")) {
            errors.rejectValue("phone1", "phone1:required", "전화번호 앞 세자리를 입력해주세요.");
        }
