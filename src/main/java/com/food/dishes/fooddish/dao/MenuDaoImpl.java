...
import com.food.dishes.fooddish.exception.menu.*;
import com.food.dishes.fooddish.mapper.MenuMapper;
...
public class MenuDaoImpl implements MenuDao {
    ...
    public int insert(MenuVO menu, StoreVO store) throws MenuException {
        ...
        int result = 0;
        try {
            result = jTemp.update(sql, store.getId(), menu.getId(), menu.getOfferType().getId(), menu.getTag(), menu.getName(), menu.getPrice(), menu.getAmount(), menu.getDescription(), menu.getNotice(), menu.getPhoto());
    ...
    public void delete(int menuId, int storeId) throws MenuException {
        ...
        if (result == 0) {
            throw new MenuDeleteFailedException("매장 삭제 실패");
        }
    }
